...
class CoverTimeBased(CoverEntity, RestoreEntity):
    ...
    async def _handle_state_changed(self, event):
        ...
        if self._switch_open_state == "off" and self._switch_close_state == "off":
            ...
            _LOGGER.debug(
                self._name + ": " + "open/close: on/on, turning off both switches"
            )
            self._handle_my_button()
            if event.data.get("entity_id") == self._close_switch_entity_id:
            ...
            if self._target_position != 100 and self._target_position != 0:
                _LOGGER.debug(
                    self._name + ": " + "open/close: on/off, opening to %d",
                    self._target_position,
                )
                self.tc.start_travel(self._target_position)
            else:
                _LOGGER.debug(
                    self._name + ": " + "open/close: on/off, opening to predefined 100"
                )
            ...
            if self._target_position != 100 and self._target_position != 0:
                _LOGGER.debug(
                    self._name + ": " + "open/close: off/on, closing to %d",
                    self._target_position,
                )
                self.tc.start_travel(self._target_position)
            else:
                _LOGGER.debug(
                    self._name + ": " + "open/close: off/on, closing to predefined 0"
                )
                self._target_position = 0
